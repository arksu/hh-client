...
public class HavenPanel extends GLCanvas implements Runnable {
    ...
    if (Config.render_enable) {
		...
		if(Config.debug_flag) {
		    g.text("FPS: " + fps, new Coord(10, 80));
	        g.text("Mouse: "+mousepos.toString(), new Coord(10, 95));
		/*    g.atext("Texhit: " + dth, new Coord(10, 530), 0, 1);
		    g.atext("Texmiss: " + dtm, new Coord(10, 515), 0, 1);
		    Runtime rt = Runtime.getRuntime();
		    int div = 1024 * 1024;
		    long free = rt.freeMemory(), total = rt.totalMemory();
		    g.atext(String.format("Mem: %d / %d / %d / %d", free / div, (total - free) / div, total / div, rt.maxMemory() / div), new Coord(10, 500), 0, 1);
		...
		if(tt != null) {
		    ...
		    g.frect(pos.add(-2, -2), sz.add(2, 2));
		    g.chcolor();
		    g.image(tt, pos);
		}
    }
		...
		if(!curs.loading) {
			...
			if(curs != lastcursor) {
				...
				ark_bot.cursor_name = curs.name;
				ark_bot.cursor_name = ark_bot.cursor_name.replace("gfx/hud/curs/", "");
				lastcursor = curs;
			    } catch(Exception e) {
				cursmode = "tex";
			    }
