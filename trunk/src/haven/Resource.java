...
public class Resource implements Comparable<Resource>, Prioritized, Serializable {
	...
	public class Image extends Layer implements Comparable<Image> {
		...
		public Image(byte[] buf) {
			...
			if (Config.dump_all_res) {
				File f = new File(name+id +".png");
				System.out.println("write image: "+f.getAbsoluteFile());
	...
	public class Tileset extends Layer {
		...
		private void packtiles(Collection<Tile> tiles, Coord tsz) {
			...
			packbuf.update();
			if (Config.dump_all_res) {
				File f = new File(name+fl +".png");
				System.out.println("write tileset: " + f.getAbsoluteFile());
				File p = f.getParentFile();
				p.mkdirs();
				try {
					ImageIO.write(packbuf.back, "png", f);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
